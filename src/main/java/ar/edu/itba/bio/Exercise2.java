...
public enum Exercise2 {
    ...
    private static void solveLocal(final File ignored) {
        throw new UnsupportedOperationException("[Exercise 2] Implementation for local database does not exist");
    }
    ...
    private static void solveRemote(final File fastaFile) throws Exception {
        ...
        if (proteinSequences.size() != 1) {
            throw new IllegalArgumentException("[Exercise 2] Only one protein sequence is accepted and it is assumed " +
                    "it was translated using the correct Reading Frame.");
        }
        for (final ProteinSequence onlyProteinSequence : proteinSequences.values()) {
            try (final var result = blastRequest(onlyProteinSequence.getSequenceAsString(), alignmentProperties(), outputProperties())) {
    ...
    private static InputStream blastRequest(final String sequence, final NCBIQBlastAlignmentProperties alignmentProperties,
    ...
    private static void saveToFileBlastResult(final String parent, final InputStream result) throws IOException {
        final File blastReport = Paths.get(parent, "blast.out").toFile();
        if (!blastReport.createNewFile()) {
            throw new IllegalStateException("[Exercise 2] \"blast.out\" exists in the same dir as the input file. Will not overwrite file.");
        }
